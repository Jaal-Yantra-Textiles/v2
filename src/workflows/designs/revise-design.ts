...
const cloneDesignStep = createStep(
  ...
  async (
    ...
    // Build the new design data from the original
    const newDesignData: Record<string, any> = {
      name: original.name,
    ...
    const newDesign = await designService.createDesigns(newDesignData)

    // Clone colors
    if (original.colors?.length) {
      await designService.createDesignColors(
        original.colors.map((c: any) => ({
    ...
    if (original.specifications?.length) {
      await designService.createDesignSpecifications(
        original.specifications.map((spec: any) => ({
          design_id: newDesign.id,
          title: spec.title,
    ...
    if (original.components?.length) {
      await designService.createDesignComponents(
        original.components.map((comp: any) => ({
