...
export class Migration20250202122433 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table if not exists "task_dependency" ("id" text not null, "dependency_type" text check ("dependency_type" in (\'blocking\', \'related\', \'subtask\')) not null default \'blocking\', "metadata" jsonb null, "outgoing_task_id" text not null, "incoming_task_id" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "task_dependency_pkey" primary key ("id"));');
    this.addSql('CREATE INDEX IF NOT EXISTS "IDX_task_dependency_outgoing_task_id" ON "task_dependency" (outgoing_task_id) WHERE deleted_at IS NULL;');
    this.addSql('CREATE INDEX IF NOT EXISTS "IDX_task_dependency_incoming_task_id" ON "task_dependency" (incoming_task_id) WHERE deleted_at IS NULL;');
