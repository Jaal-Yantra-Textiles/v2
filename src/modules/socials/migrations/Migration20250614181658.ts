...
export class Migration20250614181658 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table if not exists "social_platform" ("id" text not null, "name" text not null, "icon_url" text not null, "base_url" text not null, "api_config" jsonb not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "social_platform_pkey" primary key ("id"));`);
    ...
    this.addSql(`create table if not exists "social_post" ("id" text not null, "post_url" text not null, "caption" text not null, "status" text check ("status" in ('draft', 'scheduled', 'posted', 'failed', 'archived')) not null, "scheduled_at" timestamptz not null, "posted_at" timestamptz not null, "insights" jsonb not null, "media_attachments" jsonb not null, "notes" text not null, "error_message" text not null, "related_item_type" text not null, "related_item_id" text not null, "platform_id" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "social_post_pkey" primary key ("id"));`);
