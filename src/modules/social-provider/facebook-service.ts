...
export default class FacebookService {
  ...
  getAuthUrl(redirectUri: string, scope: string, state?: string): string {
    ...
    // Default to Facebook Page permissions required for listing and publishing
    // Use comma-separated scopes as required by Facebook OAuth
    const defaultScope = "pages_show_list,pages_manage_engagement,pages_manage_posts";
  ...
  initiateUserAuth(redirectUri: string, scope: string = "pages_show_list,pages_read_engagement,pages_manage_posts") {
  ...
  async exchangeCodeForToken(code: string, redirectUri: string): Promise<OAuth2Token> {
    ...
    const response = await fetch(`https://graph.facebook.com/v19.0/oauth/access_token?${params.toString()}`, { method: "GET" })
    if (!response.ok) {
  ...
  async listManagedPages(userAccessToken: string): Promise<Array<{ id: string; name?: string }>> {
    ...
    const resp = await fetch(`https://graph.facebook.com/v23.0/me/accounts?fields=id,name&access_token=${encodeURIComponent(userAccessToken)}`)
    if (!resp.ok) {
  ...
  async getPageFields<T extends Record<string, any>>(pageId: string, fields: string[], userAccessToken: string): Promise<T> {
    ...
    const resp = await fetch(`https://graph.facebook.com/v19.0/${encodeURIComponent(pageId)}?${qs.toString()}`)
    if (!resp.ok) {
      const err = await resp.json().catch(() => ({}))
      throw new MedusaError(
        MedusaError.Types.INVALID_DATA,
  ...
  async getPageAccessToken(pageId: string, userAccessToken: string): Promise<string> {
    ...
    const resp = await fetch(
      `https://graph.facebook.com/v19.0/${encodeURIComponent(pageId)}?fields=access_token&access_token=${encodeURIComponent(userAccessToken)}`,
      { method: "GET" }
    )
    if (!resp.ok) {
      const err = await resp.json().catch(() => ({}))
  ...
  async createPagePhotoPost(pageId: string, input: { message?: string; image_url?: string }, pageAccessToken: string) {
    ...
    const resp = await fetch(`https://graph.facebook.com/v19.0/${encodeURIComponent(pageId)}/photos`, {
      method: "POST",
  ...
  async createPageFeedPost(pageId: string, input: { message: string; link?: string }, pageAccessToken: string) {
    ...
    const resp = await fetch(`https://graph.facebook.com/v19.0/${encodeURIComponent(pageId)}/feed`, {
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      body: body.toString(),
    })
