...
export default async function designProductionCompletedHandler({
  event: { data },
  container,
  ...
  try {
    ...
    await sendDesignStatusUpdateEmailWorkflow(container).run({
      input: {
        ...
        templateKey: "design-production-completed",
        designStatus: "Production Complete",
        designUrl,
        extraData: {
    ...
    logger.warn(
      `[design.production_completed] Failed to send notification for design ${data.design_id}: ${(error as any)?.message || error}`
    )
  }
}

export const config: SubscriberConfig = {
  event: "design.production_completed",
}
