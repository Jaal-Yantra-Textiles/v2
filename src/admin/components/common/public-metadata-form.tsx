...
type MetaDataSubmitHook<TRes> = (
  params: { public_metadata?: Record<string, any> | null },
...
const MetadataSchema = z.object({
  public_metadata: z.array(MetadataFieldSchema),
});

export const PublicMetadataForm = <TRes,>(props: MetadataFormProps<TRes>) => {
...
const InnerForm = <TRes,>({
  ...
  const hasUneditableRows = getHasUneditableRows(public_metadata);

  const form = useForm<z.infer<typeof MetadataSchema>>({
    defaultValues: {
      public_metadata: getDefaultValues(public_metadata),
    },
                ...
                >
                  <div className="group/table relative">
                    ...
                    >
                      <Form.Field
                        control={form.control}
                        name={`public_metadata.${index}.key`}
                        render={({ field }) => {
                          return (
                            <Form.Item>
                              <Form.Control>
                      ...
                      <Form.Field
                        control={form.control}
                        name={`public_metadata.${index}.value`}
...
function parseValues(
  ...
  const metadata = values.public_metadata;
