...
export const FilePreview = ({
  ...
  // For backward compatibility with our previous implementation
  name?: string
  size?: number
  onRemove?: () => void
}) => {
  // Use either filename or name (for backward compatibility)
      ...
      <div className="flex flex-row items-center justify-between gap-2">
        ...
        {!loading && url && (
          <IconButton variant="transparent" asChild>
            <a href={url} download={displayName ?? `${Date.now()}`}>
              <ArrowDownTray />
            </a>
          </IconButton>
        )}
        {!loading && onRemove && !actions && (
