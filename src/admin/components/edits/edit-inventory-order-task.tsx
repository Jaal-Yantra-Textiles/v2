...
type EditInventoryOrderTaskFormProps = {
...
export const EditInventoryOrderTaskForm = ({ task, inventoryOrderId }: EditInventoryOrderTaskFormProps) => {
  const { t } = useTranslation();
  const { handleSuccess } = useRouteModal();
  const { mutateAsync, isPending } = useUpdateInventoryOrderTask(inventoryOrderId, task.id);
  ...
  const handleSubmit = async (data: TaskFormData) => {
    await mutateAsync(
      ...
      },
      {
        ...
        onError: (error) => {
          toast.error(
            error?.message || t("tasks.updateError", "Failed to update task")
          );
        },
      }
    );
