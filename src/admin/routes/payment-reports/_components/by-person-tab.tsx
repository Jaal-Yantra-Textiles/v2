...
export const ByPersonTab = () => {
  ...
  const { by_person = [], isPending } = usePaymentReportsByPerson(query) as any
        ...
        <Table>
          ...
          <Table.Body>
            {by_person.length === 0 ? (
              ...
              by_person.map((row: any) => (
                <Table.Row key={row.person_id}>
                  <Table.Cell>{row.person_name}</Table.Cell>
