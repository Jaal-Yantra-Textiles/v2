...
export const OverviewTab = () => {
  ...
  }, [platformId, adAccountId, level, effectiveObjectId, datePreset, includeAudience, includeContent, persist, refreshMode, maxAgeMinutes])
  ...
  useEffect(() => {
    ...
    if (isFetching) {
      if (fetchKey !== lastFetchKeyRef.current) {
        ...
        const message = refreshMode === "force" ? "Syncing latest data from Meta..." : "Fetching overview..."
  ...
  }, [platformId, adAccountId, fetchKey, isFetching, isError, error, overview, refreshMode, persist])

  const resultsRows: ResultsRow[] = useMemo(() => {
    return Object.entries(overview?.results || {})
      .map(([k, v]) => ({ action_type: k, value: v }))
  ...
  const placementsChartData = useMemo(() => {
    return (overview?.content?.by_publisher_platform || [])
      .slice(0, 8)
      .map((r) => ({
  ...
  const audienceAgeGenderChartData = useMemo(() => {
    ...
    for (const r of rows) {
      ...
      if (!byAge[age]) byAge[age] = { age, male: 0, female: 0, unknown: 0 }
      if (gender === "male") byAge[age].male += spend
      else if (gender === "female") byAge[age].female += spend
      else byAge[age].unknown += spend
    }

    return Object.values(byAge).sort((a, b) => {
      const parseAgeStart = (age: string) => {
        const s = String(age || "").trim().toLowerCase()
        if (!s || s === "unknown") return Number.POSITIVE_INFINITY
        if (s.includes("+")) { const n = parseInt(s.replace("+", ""), 10); return Number.isFinite(n) ? n : Number.POSITIVE_INFINITY }
        const n = parseInt(s.split("-")[0], 10)
        return Number.isFinite(n) ? n : Number.POSITIVE_INFINITY
      }
      const pa = parseAgeStart(a.age), pb = parseAgeStart(b.age)
      return pa !== pb ? pa - pb : String(a.age).localeCompare(String(b.age))
    })
  }, [overview])

  const audienceCountryChartData = useMemo(() => {
    return (overview?.audience?.by_country || [])
      .slice(0, 10)
  ...
  const breakdownColumns = useMemo(
    () => [
      breakdownColumnHelper.accessor("key", {
        ...
        cell: ({ getValue }) => {
          const obj = getValue() || {}
          const label = Object.entries(obj).map(([k, v]) => `${k}: ${v}`).join(", ")
  ...
  const audienceAgeGenderTable = useDataTable({ data: audienceAgeGenderRows, columns: breakdownColumns, getRowId: (_, i) => String(i), rowCount: audienceAgeGenderRows.length, isLoading })
  const audienceCountryTable = useDataTable({ data: audienceCountryRows, columns: breakdownColumns, getRowId: (_, i) => String(i), rowCount: audienceCountryRows.length, isLoading })
  const contentPublisherTable = useDataTable({ data: contentPublisherRows, columns: breakdownColumns, getRowId: (_, i) => String(i), rowCount: contentPublisherRows.length, isLoading })
  const contentPositionTable = useDataTable({ data: contentPositionRows, columns: breakdownColumns, getRowId: (_, i) => String(i), rowCount: contentPositionRows.length, isLoading })
  const contentDeviceTable = useDataTable({ data: contentDeviceRows, columns: breakdownColumns, getRowId: (_, i) => String(i), rowCount: contentDeviceRows.length, isLoading })
  ...
  return (
    <Container className="divide-y p-0">
      ...
      <div className="px-4 md:px-6 py-4">
        <div className="flex flex-col gap-3">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <Select value={platformId} onValueChange={(value) => { setPlatformId(value); setAdAccountId("") }}>
              <Select.Trigger><Select.Value placeholder="Platform" /></Select.Trigger>
              <Select.Content>
                {metaPlatforms.map((p) => <Select.Item key={p.id} value={p.id}>{p.name}</Select.Item>)}
              </Select.Content>
            </Select>

            <Select value={adAccountId} onValueChange={(value) => { setAdAccountId(value); setObjectId("") }} disabled={!platformId}>
              <Select.Trigger><Select.Value placeholder="Ad Account" /></Select.Trigger>
              <Select.Content>
                {adAccounts.map((a) => <Select.Item key={a.id} value={a.meta_account_id}>{a.name}</Select.Item>)}
              </Select.Content>
            </Select>

            <Select value={level} onValueChange={(value) => { setLevel(value as MetaAdsOverviewLevel); setObjectId("") }} disabled={!adAccountId}>
              <Select.Trigger><Select.Value placeholder="Level" /></Select.Trigger>
              <Select.Content>
                {LEVELS.map((l) => <Select.Item key={l.value} value={l.value}>{l.label}</Select.Item>)}
              </Select.Content>
            </Select>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            <Select value={datePreset} onValueChange={setDatePreset} disabled={!adAccountId}>
              <Select.Trigger><Select.Value placeholder="Date preset" /></Select.Trigger>
              <Select.Content>
                {DATE_PRESETS.map((p) => <Select.Item key={p.value} value={p.value}>{p.label}</Select.Item>)}
              </Select.Content>
            </Select>

            {level === "campaign" ? (
              <Select value={objectId} onValueChange={setObjectId} disabled={!adAccountId}>
                <Select.Trigger><Select.Value placeholder="Campaign" /></Select.Trigger>
                <Select.Content>
                  {campaignOptions.map((c) => <Select.Item key={c.value} value={c.value}>{c.label}</Select.Item>)}
                </Select.Content>
              </Select>
            ) : level !== "account" ? (
              <Input value={objectId} onChange={(e) => setObjectId(e.target.value)} placeholder="Object ID (Meta ID)" disabled={!adAccountId} />
            ) : (
              <div className="hidden lg:block" />
            )}

            <Select value={refreshMode} onValueChange={(v) => setRefreshMode(v as any)}>
              <Select.Trigger><Select.Value placeholder="Refresh" /></Select.Trigger>
              <Select.Content>
                <Select.Item value="auto">Auto</Select.Item>
                <Select.Item value="force">Force Meta</Select.Item>
                <Select.Item value="never">DB only</Select.Item>
              </Select.Content>
            </Select>

            <Input
              value={String(maxAgeMinutes)}
              onChange={(e) => { const n = Number(e.target.value); setMaxAgeMinutes(Number.isFinite(n) ? n : 60) }}
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge size="2xsmall" color={overview?.capabilities?.remote_ad_creation?.supported ? "green" : "grey"}>
              {overview?.capabilities?.remote_ad_creation?.supported ? "Remote ad creation supported" : "Remote ad creation (coming soon)"}
            </Badge>
          </div>
        </div>
      </div>
        ...
        <div className="px-4 md:px-6 py-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-ui-bg-base border border-ui-border-base rounded-lg p-4">
              <Text size="small" className="text-ui-fg-subtle">Spend</Text>
          ...
          <Container className="divide-y p-0 mt-4">
            <div className="px-4 md:px-6 py-4">
              ...
              <Text size="small" className="text-ui-fg-subtle">Top action types by volume</Text>
            </div>
            <div className="px-4 md:px-6 pb-4 md:pb-6 h-[260px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={topActionsChartData} margin={{ top: 8, right: 16, left: 0, bottom: 8 }}>
                  ...
                  <Tooltip contentStyle={{ background: "var(--ui-bg-base)", border: "1px solid var(--ui-border-base)", color: "var(--ui-fg-base)" }} />
          ...
          <Container className="divide-y p-0">
            <div className="px-4 md:px-6 py-4">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                  <Heading level="h2">
                    {groupBy === "campaign" ? "Top Campaigns" : groupBy === "adset" ? "Top Ad Sets" : "Top Ads"}
                  </Heading>
                ...
                <div className="w-full md:w-[220px]">
                  <Select value={groupBy} onValueChange={(v) => setGroupBy(v as any)}>
                    <Select.Trigger><Select.Value placeholder="Group by" /></Select.Trigger>
                    <Select.Content>
                      <Select.Item value="campaign">Campaigns</Select.Item>
                      <Select.Item value="adset">Ad Sets</Select.Item>
                      <Select.Item value="ad">Ads</Select.Item>
                    </Select.Content>
                  </Select>
                </div>
              </div>
            ...
            <div className="divide-y">
              {groupBy === "campaign" && topCampaigns.map((c: any) => (
                <button key={c.id} type="button" className="w-full text-left px-4 md:px-6 py-4 hover:bg-ui-bg-subtle transition-colors" onClick={() => navigate(`/meta-ads/campaigns/${c.id}`)}>
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 min-w-0">
                        <Text className="font-medium truncate">{c.name}</Text>
                        <Badge size="2xsmall" color={getStatusColor(c.status)}>{String(c.status || "UNKNOWN")}</Badge>
                      </div>
                      {c.objective && <Text size="xsmall" className="text-ui-fg-subtle mt-1 truncate">{String(c.objective).replace("OUTCOME_", "").replace(/_/g, " ")}</Text>}
                    </div>
                    <div className="shrink-0 text-right">
                      <Text size="small" className="font-medium">{formatNumber(Number(c.spend || 0))}</Text>
                      <Text size="xsmall" className="text-ui-fg-subtle">spend</Text>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-3">
                    <div><Text size="xsmall" className="text-ui-fg-subtle">Impressions</Text><Text size="small">{formatNumber(Number(c.impressions || 0))}</Text></div>
                    <div><Text size="xsmall" className="text-ui-fg-subtle">Reach</Text><Text size="small">{formatNumber(Number(c.reach || 0))}</Text></div>
                    <div><Text size="xsmall" className="text-ui-fg-subtle">Clicks</Text><Text size="small">{formatNumber(Number(c.clicks || 0))}</Text></div>
                    <div><Text size="xsmall" className="text-ui-fg-subtle">CTR</Text><Text size="small">{formatPercent(Number(c.ctr || 0))}</Text></div>
                  </div>
                </button>
              ))}

              {groupBy === "adset" && topAdSets.map((as: any) => (
                <button key={as.id} type="button" className="w-full text-left px-4 md:px-6 py-4 hover:bg-ui-bg-subtle transition-colors" onClick={() => navigate(`/meta-ads/overview/adsets/${as.id}`)}>
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 min-w-0">
                        <Text className="font-medium truncate">{as.name}</Text>
                        <Badge size="2xsmall" color={getStatusColor(as.status)}>{String(as.status || "UNKNOWN")}</Badge>
                      </div>
                    </div>
                    <div className="shrink-0 text-right">
                      <Text size="small" className="font-medium">{formatNumber(Number(as.spend || 0))}</Text>
                      <Text size="xsmall" className="text-ui-fg-subtle">spend</Text>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-3">
                    <div><Text size="xsmall" className="text-ui-fg-subtle">Impressions</Text><Text size="small">{formatNumber(Number(as.impressions || 0))}</Text></div>
                    <div><Text size="xsmall" className="text-ui-fg-subtle">Reach</Text><Text size="small">{formatNumber(Number(as.reach || 0))}</Text></div>
                    <div><Text size="xsmall" className="text-ui-fg-subtle">Clicks</Text><Text size="small">{formatNumber(Number(as.clicks || 0))}</Text></div>
                    <div><Text size="xsmall" className="text-ui-fg-subtle">CTR</Text><Text size="small">{formatPercent(Number(as.ctr || 0))}</Text></div>
                  </div>
                </button>
              ))}

              {groupBy === "ad" && topAds.map((a: any) => (
                <button key={a.id} type="button" className="w-full text-left px-4 md:px-6 py-4 hover:bg-ui-bg-subtle transition-colors" onClick={() => navigate(`/meta-ads/overview/ads/${a.id}`)}>
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 min-w-0">
                        <Text className="font-medium truncate">{a.name}</Text>
                        <Badge size="2xsmall" color={getStatusColor(a.status)}>{String(a.status || "UNKNOWN")}</Badge>
                      </div>
                    </div>
                    <div className="shrink-0 text-right">
                      <Text size="small" className="font-medium">{formatNumber(Number(a.spend || 0))}</Text>
                      <Text size="xsmall" className="text-ui-fg-subtle">spend</Text>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-3">
                    <div><Text size="xsmall" className="text-ui-fg-subtle">Impressions</Text><Text size="small">{formatNumber(Number(a.impressions || 0))}</Text></div>
                    <div><Text size="xsmall" className="text-ui-fg-subtle">Reach</Text><Text size="small">{formatNumber(Number(a.reach || 0))}</Text></div>
                    <div><Text size="xsmall" className="text-ui-fg-subtle">Clicks</Text><Text size="small">{formatNumber(Number(a.clicks || 0))}</Text></div>
                    <div><Text size="xsmall" className="text-ui-fg-subtle">CTR</Text><Text size="small">{formatPercent(Number(a.ctr || 0))}</Text></div>
                  </div>
                </button>
              ))}
            </div>
      ...
      {/* Results table — only when data exists */}
      {resultsRows.length > 0 && (
        <>
          <div className="px-4 md:px-6 py-4">
            <Heading level="h2">Results (all actions)</Heading>
            <Text size="small" className="text-ui-fg-subtle">Aggregated counts per action type from Meta insights actions array</Text>
          </div>
      ...
      {includeAudience && overview?.audience && (
        <>
            ...
            <div className="px-4 md:px-6 pb-4 md:pb-6 grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
              <Container className="divide-y p-0">
                <div className="px-4 md:px-6 py-4">
                  ...
                  <Text size="small" className="text-ui-fg-subtle">Stacked spend distribution</Text>
                </div>
                <div className="px-4 md:px-6 pb-4 md:pb-6 h-[260px]">
                  <ResponsiveContainer width="100%" height="100%">
              ...
              <Container className="divide-y p-0">
                <div className="px-4 md:px-6 py-4">
                  ...
                  <Text size="small" className="text-ui-fg-subtle">Highest spend countries</Text>
                </div>
                <div className="px-4 md:px-6 pb-4 md:pb-6 h-[260px]">
                  <ResponsiveContainer width="100%" height="100%">
          ...
          <div className="px-4 md:px-6 pb-4 md:pb-6 grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
            <Container className="divide-y p-0">
              <div className="px-4 md:px-6 py-4"><Heading level="h3">Age & Gender</Heading></div>
              <DataTable instance={audienceAgeGenderTable}><DataTable.Table /></DataTable>
            </Container>
            <Container className="divide-y p-0">
              <div className="px-4 md:px-6 py-4"><Heading level="h3">Country</Heading></div>
              <DataTable instance={audienceCountryTable}><DataTable.Table /></DataTable>
            </Container>
          </div>
        </>
      )}
      ...
      {includeContent && overview?.content && (
        <>
            ...
            <div className="px-4 md:px-6 pb-4 md:pb-6">
              <Container className="divide-y p-0">
                <div className="px-4 md:px-6 py-4">
                  ...
                  <Text size="small" className="text-ui-fg-subtle">Top platforms by spend</Text>
                </div>
                <div className="px-4 md:px-6 pb-4 md:pb-6 h-[260px]">
                  <ResponsiveContainer width="100%" height="100%">
          ...
          <div className="px-4 md:px-6 pb-4 md:pb-6 grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
            <Container className="divide-y p-0">
              <div className="px-4 md:px-6 py-4"><Heading level="h3">Publisher Platform</Heading></div>
              <DataTable instance={contentPublisherTable}><DataTable.Table /></DataTable>
            </Container>
            <Container className="divide-y p-0">
              <div className="px-4 md:px-6 py-4"><Heading level="h3">Position</Heading></div>
              <DataTable instance={contentPositionTable}><DataTable.Table /></DataTable>
            </Container>
            <Container className="divide-y p-0">
              <div className="px-4 md:px-6 py-4"><Heading level="h3">Device</Heading></div>
              <DataTable instance={contentDeviceTable}><DataTable.Table /></DataTable>
            </Container>
          </div>
        </>
      )}
