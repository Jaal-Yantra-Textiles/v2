...
export const FolderMediaGallery = ({ folder }: FolderMediaGalleryProps) => {
  ...
  const media = getMedia(folder.media_files || [])
  ...
  const handleDownloadCurrent = () => {
    ...
    a.download = media[curr].file_name || "file"
    a.target = "_blank"
    a.click()
  }

  const handleDeleteCurrent = async () => {
    // Deleting individual media in folder context can be added later if API exists
    // For now, no-op
  }
      ...
      <RouteFocusModal.Header>
        <div className="flex items-center justify-end gap-x-2">
          ...
          <IconButton size="small" type="button" onClick={handleDownloadCurrent} disabled={noMedia}>
            <ArrowDownTray />
            <span className="sr-only">{t("media.downloadImageLabel", "Download image")}</span>
          </IconButton>
...
const Canvas = ({ media, curr }: { media: MediaView[]; curr: number }) => {
  const { t } = useTranslation()
  const { goToEdit } = useFolderMediaViewContext()
      ...
      <div className="bg-ui-bg-subtle flex size-full flex-col items-center justify-center gap-y-4 pb-8 pt-6">
        <div className="flex flex-col items-center">
          <Text size="small" leading="compact" weight="plus" className="text-ui-fg-subtle">
            {t("media.emptyState.header", "No media files available")}
          </Text>
          <Text size="small" className="text-ui-fg-muted">
            {t("media.emptyState.description", "Upload media files to this folder")}
          </Text>
        </div>
        <Button size="small" variant="secondary" onClick={goToEdit}>
          {t("media.emptyState.action", "Upload media files")}
        </Button>
      </div>
    )
  }
        ...
        <div className="relative inline-block max-h-full max-w-full">
          {media[curr].is_thumbnail && (
            <div className="absolute left-2 top-2">
              <Tooltip content={t("media.thumbnailTooltip", "This is the thumbnail image")}>
                <ThumbnailBadge />
              </Tooltip>
            </div>
          )}
...
const Preview = ({
  ...
  const getVisibleItems = (media: MediaView[], index: number) => {
    if (media.length <= MAX_VISIBLE_ITEMS) return media
    ...
    if (end < start) return [...media.slice(start), ...media.slice(0, end)]
    else return media.slice(start, end)
  }
...
const getMedia = (mediaFiles: MediaFile[] = []): MediaView[] => {
  if (!mediaFiles.length) return []
  return mediaFiles.map((m) => ({
    id: m.id!,
    url: m.file_path,
    file_name: (m as any).file_name,
    is_thumbnail: Boolean(m.metadata?.is_thumbnail || (m as any).isThumbnail),
  }))
}

export default FolderMediaGallery
