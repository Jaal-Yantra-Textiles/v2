...
setupSharedTestSuite(() => {
    ...
    describe("POST /admin/inventory-orders", () => {
      ...
      /**
       * #1617 — `agreed_total` is what the partner will actually be PAID, and
       * it is not `total_price`. One real order was priced at ₹63,375.75 and
       * agreed at ₹35,000; the payout guard sums live claims against this, so
       * a ₹30,000 tranche no longer locks out the ₹5,000 balance.
       *
       * 🔴 Asserted on the READ-BACK, not on the 201. A field the validator
       * accepts and the write silently drops is the exact shape of the
       * façade this module has produced before: the request succeeds, the
       * screen looks right, and the column stays null.
       */
      it("stores an agreed_total distinct from the ordered total", async () => {
        ...
        const id = res.data.inventoryOrder.id;
        const read = await api.get(`/admin/inventory-orders/${id}`, headers);
        expect(read.status).toBe(200);
        expect(Number(read.data.inventoryOrder.agreed_total)).toBe(150);
        // The ordered total is untouched — they are two different facts.
        expect(Number(read.data.inventoryOrder.total_price)).toBe(200);
      });

      /**
       * Absent must stay absent. Defaulting it to the ordered total here would
       * make every order look like somebody agreed a price, and the payout
       * guard could never tell a recorded agreement from a guess.
       */
      it("leaves agreed_total unset when none is sent", async () => {
        const res = await api.post(
          "/admin/inventory-orders",
          {
            order_lines: [
        ...
        const read = await api.get(
          `/admin/inventory-orders/${res.data.inventoryOrder.id}`,
          headers
        );
        expect(read.data.inventoryOrder.agreed_total ?? null).toBeNull();
      });
