...
setupSharedTestSuite(() => {
  describe("Production Run → Design Status Transitions", () => {
    ...
    async function createPartner(unique: number) {
      const email = `design-status-partner-${unique}@jyt.test`
      ...
      const res = await api.post(
        "/partners",
        {
          name: `Design Status Partner ${unique}`,
          handle: `design-status-partner-${unique}`,
    ...
    async function createTemplates(adminHeaders: any, unique: number) {
      const cuttingName = `ds-cutting-${unique}`
      const stitchingName = `ds-stitching-${unique}`
    ...
    it("should transition design through full lifecycle: start → In_Development, finish → Revision, complete → Technical_Review", async () => {
      ...
      const designRes = await api.post(
        "/admin/designs",
        {
          name: `Status-Transition-Design ${unique}`,
          description: "Test design status transitions through production run lifecycle",
      ...
      // Create + dispatch production run
      ...
      // Start → design stays In_Development (production run, not sample)
      ...
      expect(designAfterStart.data.design.status).toBe("In_Development")

      // Finish → design transitions to Revision
      await api.post(`/partners/production-runs/${runId}/finish`, {}, { headers: partnerHeaders })

      const designAfterFinish = await api.get(`/admin/designs/${designId}`, adminHeaders)
      ...
      const designAfterComplete = await api.get(`/admin/designs/${designId}`, adminHeaders)
      expect(designAfterComplete.data.design.status).toBe("Technical_Review")
    })
  })
})
