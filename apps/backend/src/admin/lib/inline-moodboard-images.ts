/**
 * Turn a moodboard scene's remote image URLs into real `data:` URIs (#2228) — ADMIN COPY.
 *
 ...
 * before it starts; the proxy route is same-origin here.
 *
 * ⚠️ A COPY of `apps/partner-ui/src/lib/inline-moodboard-images.ts`, not an
 * import: the admin ships inside the backend bundle and partner-ui is a
 * separate package, so there is no module either can reach. Same reason
 * `first-media-url` exists three times in this repo. If one is changed, change
 * both — the pure part is small on purpose.
 */
