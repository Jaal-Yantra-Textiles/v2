...
const MetaAdsAdOverviewModalPage = () => {
  const { id } = useParams()
  ...
  const { data: adData, isLoading: isAdLoading } = useAd(id || "")

  const ad: any = adData?.ad
  const adAccount = ad?.ad_account
  ...
  const isLoading = isAdLoading || isOverviewLoading
    ...
    <RouteFocusModal prev="../..">
      <RouteFocusModal.Header>
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <Heading className="truncate">Ad Overview</Heading>
            {ad?.name && (
          ...
          <div className="flex items-center gap-2">
            {ad?.status && (
              <Badge size="2xsmall" color={getStatusColor(ad.status)}>
                {String(ad.status)}
              </Badge>
            )}
      ...
      <RouteFocusModal.Body className="overflow-y-auto">
        <div className="p-4 md:p-6 flex flex-col gap-4 md:gap-6">
          ...
          {!isLoading && !ad && (
            <Container className="p-6">
              <Text className="text-ui-fg-subtle">Ad not found.</Text>
            </Container>
          )}
        </div>
...
export default MetaAdsAdOverviewModalPage
