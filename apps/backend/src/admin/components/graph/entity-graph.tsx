...
/**
 * An entity and its neighbours as a graph (#1847).
 *
 ...
 *
 * An absent edge is the thing a stack of sections could never show: a section
 * renders what IS there, and this is about what is not.
 *
 * Spine-agnostic on purpose. It knows nothing about designs — the centre node,
 * its label and every neighbour come from the server's spine resolver, so
 * pointing it at a new spine is a prop change, not a new component.
 */
...
export const EntityGraph = ({ spine, id, title = "Graph" }: Props) => {
  const { graph, isLoading, isError, error } = useEntityGraph(spine, id)
  // 🔴 Unique per instance: these marker ids land in the DOM, and step 2
  //    renders this same component in a full-screen modal ON TOP of a page
  //    that may already have one. Duplicate ids make the second graph
  //    silently borrow the first's arrowheads.
  ...
  // The centre's key comes from the resolver ("design", "partner", …); the
  // edges are all `from` it, so nothing here may hardcode a spine name.
  ...
  const edgeFor = (key: string): GraphEdge | undefined =>
  ...
  const activeEdge =
    active && active.key !== spineKey ? edgeFor(active.key) : undefined
      ...
      <div className="flex flex-col lg:flex-row">
        ...
        <div className="flex-1 overflow-x-auto">
          <div className="relative" style={{ height, minWidth: width }}>
            ...
            <button
              type="button"
              onClick={() => setSelected(spineKey)}
              className={`${stateStyles("present", activeKey === spineKey)} bg-ui-bg-subtle`}
