/**
 * Admin Assistant markdown renderer (#1092 — UI clarity polish).
 *
 * The model replies in GitHub-flavoured markdown. Rendering it raw (as
 * `whitespace-pre-wrap`) leaks `#`, `|`, `**` and turns tabular data into
 * unreadable pipe soup. This parses GFM and maps every element to a
 * Medusa-UI–styled node, so prose, lists, code and — crucially — tables read
 * as native admin UI. Tables become a real `@medusajs/ui` Table.
 *
 * Ported from the partner-portal renderer (apps/partner-ui) so both assistants
 * share one look.
 */
