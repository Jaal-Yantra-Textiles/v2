...
/**
 * Book a carrier shipment for an order.
 *
 * Lifted out of `widgets/order-carrier-shipment.tsx` and into a RouteDrawer
 * (#1451 follow-on). It is a long form — rates, dimensions, pickup slot, label,
 * manual AWB — and as a flat section it pushed the rest of the order detail
 * page below the fold whether or not anyone was shipping anything today. The
 * drawer keeps the order page readable and gives the form its own URL, so a
 * half-filled booking survives a refresh and can be linked to.
 *
 * Takes an `orderId` rather than the widget's `DetailWidgetProps`, because it
 * now has two callers: the drawer route and (for its summary) the widget.
 */
export const CarrierShipmentForm = ({ orderId }: { orderId: string }) => {
...
export default CarrierShipmentForm
