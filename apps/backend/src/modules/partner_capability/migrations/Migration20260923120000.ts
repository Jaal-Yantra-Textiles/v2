 ...
 * becomes a source; knowledge and website scans get their own tables.
 *
...
export class Migration20260923120000 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`alter table if exists "partner_capability_sample" add constraint "partner_capability_sample_source_check" check ("source" in ('wizard', 'assistant', 'whatsapp', 'admin', 'website'));`);

    this.addSql(`create table if not exists "partner_capability_knowledge" ("id" text not null, "partner_id" text not null, "sample_id" text null, "fact" text not null, "source" text check ("source" in ('wizard', 'assistant', 'whatsapp', 'admin', 'website')) not null default 'admin', "source_url" text null, "observed_at" timestamptz not null, "metadata" jsonb null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "partner_capability_knowledge_pkey" primary key ("id"));`);
    ...
    this.addSql(`create table if not exists "partner_website_scan" ("id" text not null, "partner_id" text not null, "url" text not null, "origin" text not null, "platform" text check ("platform" in ('shopify', 'html')) not null, "status" text check ("status" in ('proposed', 'committed')) not null default 'proposed', "proposal" jsonb not null, "committed_at" timestamptz null, "committed_keys" jsonb null, "metadata" jsonb null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "partner_website_scan_pkey" primary key ("id"));`);
    this.addSql(`CREATE INDEX IF NOT EXISTS "IDX_partner_website_scan_deleted_at" ON "partner_website_scan" ("deleted_at") WHERE deleted_at IS NULL;`);
    this.addSql(`CREATE INDEX IF NOT EXISTS "idx_partner_website_scan_partner" ON "partner_website_scan" ("partner_id") WHERE deleted_at IS NULL;`);
  }

  override async down(): Promise<void> {
    this.addSql(`drop table if exists "partner_website_scan" cascade;`);
    this.addSql(`drop table if exists "partner_capability_knowledge" cascade;`);
    // A 'website' row would violate the narrower check, so they go first.
    this.addSql(`delete from "partner_capability_sample" where "source" = 'website';`);
    this.addSql(`alter table if exists "partner_capability_sample" drop constraint if exists "partner_capability_sample_source_check";`);
