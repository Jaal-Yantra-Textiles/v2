...
export class Migration20260730101243 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table if not exists "campaign_attribution" ("id" text not null, "analytics_session_id" text not null, "visitor_id" text not null, "website_id" text not null, "ad_campaign_id" text null, "ad_set_id" text null, "ad_id" text null, "platform" text check ("platform" in ('meta', 'google', 'generic', 'direct')) not null default 'direct', "utm_source" text null, "utm_medium" text null, "utm_campaign" text null, "utm_term" text null, "utm_content" text null, "is_resolved" boolean not null default false, "resolution_confidence" real null, "resolution_method" text check ("resolution_method" in ('exact_utm_match', 'fuzzy_name_match', 'manual', 'unresolved')) not null default 'unresolved', "entry_page" text null, "session_pageviews" integer not null default 1, "attributed_at" timestamptz not null, "session_started_at" timestamptz not null, "metadata" jsonb null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "campaign_attribution_pkey" primary key ("id"));`);
