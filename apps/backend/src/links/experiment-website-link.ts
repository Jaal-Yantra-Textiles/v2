import { defineLink } from "@medusajs/framework/utils"
import AdPlanningModule from "../modules/ad-planning"
import WebsiteModule from "../modules/website"

/**
 * Read-only link from ABExperiment to Website
 *
 * Enables graph queries like:
 * - Get experiment with website details
 * - Get website with all its A/B experiments
 */
export default defineLink(
  {
    linkable: WebsiteModule.linkable.website,
    field: "id",
    isList: true,
  },
  {
    ...AdPlanningModule.linkable.aBExperiment.id,
    primaryKey: "website_id",
  },
  {
    readOnly: true,
  }
)
