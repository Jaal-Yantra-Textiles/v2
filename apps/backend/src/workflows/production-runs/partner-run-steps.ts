 ...
 *
 * The resolution itself lives in `lib/partner-location.ts` because the
 * goods-transfer path needs the identical answer — see the note there on why a
 * second copy of this walk was worse than one wrong one. #2053
 */
export const resolvePartnerLocationStep = createStep(
  ...
  async (
    ...
    const result = await resolvePartnerLocation(container, input.partner_id)
