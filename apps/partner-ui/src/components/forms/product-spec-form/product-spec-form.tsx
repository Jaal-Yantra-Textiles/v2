...
/**
 * The product-spec editor (#1342) — what a partner agrees to make before taking
 * a custom order: the weave, its measured parameters, the colour palette, and
 * any spec the catalog does not cover.
 *
 * Fully CONTROLLED, and deliberately host-agnostic: the same component is the
 * fifth step of the product-create wizard (where there is no product yet, so
 * the value lives in the wizard's state until the product exists) and the
 * editable section on product detail (where it is loaded and saved directly).
 * A spec written at create time and a spec written afterwards must be the same
 * spec — two editors would drift, and the one the partner uses less would rot.
 *
 * Parameter inputs are generated from the catalog the BACKEND validates
 * against, so a range shown here is the range enforced on write.
 */
