 ...
 * segments (an empty ring + centre label), so the investor's Cap table shows a
 * chart by default before they have participated.
 */
export const OwnershipDonut = ({
  ...
  return (
    <div className="flex flex-col items-center gap-y-4 sm:flex-row sm:gap-x-8">
          ...
          {hasData ? (
            arcs.map((a, i) => (
              <path
                ...
                opacity={a.highlight === false ? 0.6 : 1}
              />
            ))
          ) : (
            <circle
      ...
      <div className="flex w-full flex-col gap-y-1.5">
        {hasData ? (
          ...
          <Text size="small" className="text-ui-fg-subtle">
            No allocations yet. Your ownership will appear here once your
            participation is confirmed and paid.
          </Text>
        )}
      </div>
    </div>
