...
> ### ▶ HANDOFF (next session) — retire legacy design/inventory order surfaces, then UI polish
>
> Per the user (2026-06-15), after PR #401 lands:
> 1. **Remove the legacy design + inventory order surfaces** now consolidated under `/orders`. Inventory: the `/inventory-orders` LIST + its sidebar nav (`:id` detail already redirects). Design *orders*: any remaining duplication with `/orders/design`. **RECONCILE:** `/designs` was KEPT this session as the self-serve **design authoring** hub (create + start-production live there; a partner-owned design has no order until production starts) — so "remove design orders" means the legacy *order/work* views, NOT the authoring hub. Confirm scope with the user before deleting `/designs`.
> 2. **"Design orders" terminology polish** (copy/UX, not structural): relabel the self-serve "Start production" → "Create design order"; on a design's page, surface its resulting design orders. The data already converts (production-run → unified `order` kind=design).
> 3. **Other UI enhancements** (user's stated next focus). Known small ones: repoint legacy `/inventory-orders` list rows for a direct hop (currently via the redirect); make `ConfigurableOrderListTable` (view_configurations flag) kind-aware so work-order rows don't 404.
