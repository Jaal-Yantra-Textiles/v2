...
| **PR-UI** | — | **Fold bespoke design/inventory work-order detail + actions INTO the unified `/orders/:id`** | **DONE — branch `feat/342-partner-ui-unified-workorder-detail`** (2026-06-15). Both kinds open the unified detail with the right work surface; retail-only sections hidden for work-orders. Spec `orders-unification-partner-detail.spec.ts` 4/4 + list-filter 4/4 regression-green; partner-ui `vite build` clean. |
