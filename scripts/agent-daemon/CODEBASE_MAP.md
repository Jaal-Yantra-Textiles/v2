...
- **#452 artwork-rating page (PR #716 merged, #720 OPEN):** pure lib
  `src/workflows/feedback/lib/artwork-feedback.ts` (`selectArtworkChoices(pool,seed,count)` seeded
  FNV-1a→mulberry32 N-distinct = SAME algo as `GET /web/media`; `buildArtworkPickUpdate`,
  `normalizeRatingValue`, `resolveArtworkSourceId`). Public route `src/api/web/feedback/[id]/route.ts`
  GET/POST (feedback id = token, no auth). **#720 — artwork pool source:** `resolveArtworkSourceId(env)`
  → `string|null`; **null = general public-image pool via `listAllMediasWorkflow`** (default, no album
  needed); `FEEDBACK_ARTWORK_ALBUM_ID` set = scope to Album→Folder. **Recipe — fetch a general media
  pool server-side:** `listAllMediasWorkflow(req.scope).run({input:{filters:{is_public:true,
  file_type:"image"},config:{take,skip}}})` then `result.media_files`. ⚠ that workflow whitelists
  `file_type` (MediaFile col), NOT `type` — `GET /web/media`'s unscoped `filters.type` is silently dropped.
...
- **`apps/storefront` is the LIVE cicilabel.com storefront** (NOT the `-starter` submodule). Fully Cici-Label-branded: `src/app/layout.tsx` (title default "Cici Label - …", template `"%s | Cici Label Store"`, Org/WebSite JSON-LD, `metadata.icons` favicon), homepage metadata `src/app/[countryCode]/(main)/page.tsx` `generateMetadata`, canonical/hreflang `src/lib/util/seo.ts` (`buildLocalizedAlternates` returns PATH-only — host comes from `metadataBase`), base-URL `src/lib/util/env.ts` `getBaseURL()` (env priority `NEXT_PUBLIC_BASE_URL`→`VERCEL_PROJECT_PRODUCTION_URL`→`VERCEL_URL`→localhost), `src/app/{robots,sitemap}.ts` (both use getBaseURL). Featured rails: `src/modules/home/components/featured-products/product-rail/index.tsx`.
- **To change the storefront canonical/SEO HOST: set env `NEXT_PUBLIC_BASE_URL`** (top-priority in getBaseURL) — drives canonical+sitemap+robots+JSON-LD. `next.config.js` `buildCanonicalRedirects()` DERIVES `www./shop.→apex` 301s from that same env (inert when unset). Do NOT hardcode hosts. (#734 / PR #738; hardcoded `shop.cicilabel.com` in `lib/data/website.ts:31`/`pages/[slug]/page.tsx:9`/`newsletter.ts:22` are backend website-LOOKUP keys, NOT canonical — leave them.)
- Gotchas: macOS has no `timeout`; `node next.config.js` runs `check-env-variables.js` which exits → test config logic standalone. storefront `tsc` has ~332 pre-existing errors behind `ignoreBuildErrors` (241 = `Suspense cannot be used as a JSX component` React-types) — grep your changed files only.
...
## Prod infra / env vars (AWS Copilot)
- Prod env vars live in `deploy/aws/copilot/{medusa-server,medusa-worker}/manifest.yml`.
  Non-secret plain values → under `variables:` (e.g. NODE_ENV, MEDUSA_WORKER_MODE, PARTNER_APP_URL).
  SSM Parameter Store / Secrets Manager refs → under `secrets:` (`KEY: /jyt/prod/KEY`).
- server = `MEDUSA_WORKER_MODE: server` (runs HTTP routes incl. webhooks); worker = `MEDUSA_WORKER_MODE: worker` (runs subscribers + scheduled jobs). Code reachable from BOTH paths needs the var on BOTH manifests.
- To add a non-secret prod env: edit `variables:` in BOTH manifests (2-space indent), it deploys on merge via deploy-to-aws.yml. New SSM secret also needs copilot-application=jyt + copilot-environment=prod tags (see memory).
- WhatsApp partner-product link: `buildPartnerProductUrl` in `apps/backend/src/workflows/whatsapp/partner-product-url.ts` reads PARTNER_APP_URL → PARTNER_PORTAL_URL → hardcoded default. Triggered from server (`src/api/webhooks/social/whatsapp/route.ts`) + worker (`src/subscribers/visual-flow-event-trigger.ts`).

## #457 seed jobs (Data Plumbing seeding)
- Registry: `apps/backend/src/api/admin/ops/maintenance-jobs/registry.ts` (MAINTENANCE_JOBS array + MaintenanceJob type ~L106).
- Seed jobs live in `.../maintenance-jobs/seed-jobs.ts` (separate from registry; registry imports the job value, seed-jobs imports `type` only → no cycle).
- To expose another seed: export its DATA from the seed script (`export const ...`), import it in seed-jobs.ts, add a pure planner (partition by natural key), register the job in MAINTENANCE_JOBS. Dry-run MUST write nothing (don't call the seed's default fn — it always writes).
- Email-template seeds wrapped by ONE job `seed-email-templates` (set param). Existing-key check: `listEmailTemplates({},{select:["template_key"],take:100000})`.
- Unit tests: `__tests__/*.unit.spec.ts`, run via `TEST_TYPE=unit npx jest <path>`.

## visual_flows — live execute-visual-flow driver internals (#622 parity gate)
- **Live driver:** `apps/backend/src/workflows/visual-flows/execute-visual-flow.ts` — `executeOperationsStep` (:257-378) builds graph inline from `flow.canvas_state` (DB ops fallback per-key); `executeOperationsRecursive` (:438-506) = DFS + `visited` Set (#468 dedup :447-464, position-sort :451-454); `executeSingleOperation` (:511-618) = raw opts to handler / interpolated opts to log row, running→success/failure rows, `describeFetchError(error)` (:602, #704), `operation_id=db_operation_id` (#704, resolved :303-308); `findNextOperations` (:624-650) condition `_branch` routing; init seeds dataChain + lifecycle `started` event (:111-169); compensation digs failure log → `failed` event (:188-251).
- **Compiler (#463 target path):** `apps/backend/src/modules/visual_flows/compiler.ts` — `extractGraph`(:61-110) mirrors driver graph build, `compileFlow`(:134-259) Kahn topo→levels+cycle/unknown-type/dangling-edge hard errors, option mismatch=warning. `CompiledNode` (:25-35) DROPS position_x/y + db_operation_id (gaps to close before rewrite). Persist: `service.compileAndPersistPlan` (`service.ts:38-59`), columns `models/visual-flow.ts:29-30`, save step `visual-flow-steps.ts:264`, Redis cache `flow-plan-cache.ts`.
- **Durable wait primitive (NOT wired into executor):** `flow-wait.ts` (async step + setStepSuccess), resume route `api/admin/visual-flows/waits/[transaction_id]/resume/route.ts`; live `wait_for_event` op = marker-only (`operations/wait-for-event.ts:46-58`).
- **6 invocation sites of `executeVisualFlowWorkflow`:** workflows/visual-flows/index.ts, api/admin/visual-flows/[id]/execute/route.ts, api/webhooks/flows/[id]/route.ts, subscribers/visual-flow-event-trigger.ts, jobs/run-scheduled-visual-flows.ts, operations/trigger-flow.ts (nested).
- **Parity gate doc:** `apps/docs/notes/622_VFLOW_DRIVER_PARITY.md` (PR #723) — checklist every box the compiled driver must satisfy + shadow-run plan. Read it before touching the driver. Design: `apps/docs/notes/VISUAL_FLOWS_P1_ENGINE_DESIGN.md` (#459 build order).

## #460 email leads (inbound email → Lead) — chunk added 2026-06-24
- Backend root is `apps/backend/src` (NOT `src`; CLAUDE.md paths are stale).
- socials `Lead` model: `apps/backend/src/modules/socials/models/Lead.ts`; CRUD `socials.{create,list,update}Leads` (SOCIALS_MODULE="socials"). `meta_lead_id`+`created_time` NOT nullable.
- inbound emails: module `apps/backend/src/modules/inbound_emails` (InboundEmail has `folder`/`from_address`/`subject`/`message_id`/`imap_uid`/`status`); written by `subscribers/imap-sync.ts` (iCloud IMAP) + `api/webhooks/inbound-email/resend/route.ts`. NO create event emitted.
- Visual-flow event trigger: `subscribers/visual-flow-event-trigger.ts` — emit ANY event name; flow matches via event_pattern/event_types/event_type. To drive follow-up with no code, emit a custom event.
- To add a "react to email landing in a folder" feature: scheduled job scanning `listInboundEmails` + pure selector (see `workflows/leads/lib/email-lead.ts`), NOT a subscriber (no event on inbound-email create).
- feedback-reminder PATTERN to mirror (selector+job+stamp+fail-soft): `jobs/send-feedback-reminders.ts` + `workflows/feedback/lib/feedback-reminder.ts`.

## Partner onboarding wizard + onboarding profile (#648 slice 1)
- REAL partner onboarding wizard: `apps/partner-ui/src/routes/home/home-onboarding/home-onboarding.tsx` (RouteFocusModal + ProgressTabs). Routed via `apps/partner-ui/src/dashboard-app/routes/get-partner-route.map.tsx` path `onboarding` → `home/home-onboarding`. NOTE `apps/partner-ui/src/components/onboarding/onboarding-modal.tsx` is DEAD (zero imports) — don't edit it.
- Onboarding profile module: `apps/backend/src/modules/partner-onboarding-profile/` (mirror of `partner-payment-config`). Register new partner modules in BOTH `medusa-config.ts` + `medusa-config.prod.ts`.
- Onboarding profile API: `apps/backend/src/api/partners/onboarding-profile/{route.ts,validators.ts}`; GET/PUT, upsert pattern. Middleware: import validator + add matcher in `apps/backend/src/api/middlewares.ts` near `/partners/update` (~line 734).
- To add a partner-scoped typed-column feature: new module (model+service+index+hand-written `text check` migration) → register in both configs → `/partners/<x>` route using `getPartnerFromAuthContext` (helpers.ts) → middleware matcher → integration spec mirroring `partner-tax-pricing-api.spec.ts` (register→POST /partners→re-login for partner bearer context).

## #728 design↔inventory photo (admin UI)
- Add-Inventory link table: `apps/backend/src/admin/components/designs/design-inventory-table.tsx` → columns in `components/designs/hooks/use-inventory-columns.tsx`; nav `/app/designs/{id}/addinv` (route `routes/designs/[id]/@addinv/page.tsx`).
- Link drawer: `routes/designs/[id]/@inventory/[inventoryId]/page.tsx`; data via `useDesignInventory` → `listDesignInventoryWorkflow` (`workflows/designs/inventory/list-design-inventory.ts`) fetches `inventory_item.raw_materials.*`.
- raw_material media: model `modules/raw_material/models/raw_material.ts:37` `media: model.json()`; persisted as `{ files: string[] }` (form `components/forms/raw-material/raw-material-form.tsx:172`; bulk-import normalize `api/admin/inventory-items/bulk-import/route.ts:87`). To render a material photo, use `lib/utils/first-media-url.ts` `firstMediaUrl()` + `components/common/thumbnail.tsx` `<Thumbnail>`.
- To render any admin thumbnail with placeholder fallback: `<Thumbnail src={url} size="small|base|large" />` (Cloudflare resize, `<Photo>` fallback; passes non-CF urls through).

## #729 storefront production story (PR #732)
- Backend root is `apps/backend/src` (NOT repo-root `src/`). Storefront `apps/storefront/src`.
- Public design story route: `apps/backend/src/api/store/custom/designs/[id]/production-story/{route.ts,build-story.ts}` (+ `__tests__/build-story.unit.spec.ts`). NO auth — public store route.
- Production runs link to design by FLAT `production_run.design_id` text column (`src/modules/production_runs/models/production-run.ts`), NOT a module link → fetch `query.graph({entity:"production_runs",filters:{design_id}})`. Activity model: `production-run-activity.ts` (filter by `production_run_id`).
- Design↔consumption_log link: `src/links/design-consumption-log.ts` (relation `consumption_logs`); consumption_log has flat `design_id` too → fetch `entity:"consumption_log",filters:{design_id}`. Energy/labor/material buckets defined in `src/modules/production_runs/cost-summary.ts`.
- Design↔person link: `src/links/designs-person-link.ts` (relation `persons`, extra col `role`). Design↔partner: `design-partners-link.ts` (relation `partners`).
- Raw materials: `inventory_items.raw_materials.*` (+ `.material_type.*`); `raw_materials` arrives as a SINGLE object from query.graph (belongsTo) — normalize to array. Model `src/modules/raw_material/models/raw_material.ts` (media json, composition, color, unit_cost).
- Storefront product fetch fields: `apps/storefront/src/lib/data/products.ts:~73`. Design data lib + new `getProductionStory`: `apps/storefront/src/lib/data/designs.ts`.
- Store-route field allowlist: `/store/products` GET uses `queryConfig.fields` (passed via `?fields=`); `+designs.X.*` paths flow straight to `query.graph`. To add story sub-fields, just extend the `fields` string (no separate allowlist edit needed — confirmed working for existing designs.* fields).
- `/store/*` integration tests need `x-publishable-api-key`: `createApiKeysWorkflow` + `linkSalesChannelsToApiKeyWorkflow` from `@medusajs/medusa/core-flows`, link to store default_sales_channel.

## #729 storefront production-story RENDER (PR #741, render follow-up to #732)
- Product page render component: `apps/storefront/src/modules/products/components/production-story/` — `index.tsx` (async server component, `await getProductionStory(designId)`), `lib.ts` (pure fmt), `skeleton.tsx`, `__tests__/lib.test.ts`.
- Mount point: `apps/storefront/src/modules/products/templates/index.tsx` (`ProductTemplate`, a NON-async React.FC). `const design = product.designs?.[0]`; renders `<DesignInfo>` TWICE (desktop sticky col + mobile `small:hidden` block) — add new product-page sections next to BOTH, gated `design?.id`, wrap async children in `<Suspense fallback={<Skeleton/>}>`.
- Style template to MIRROR: `apps/storefront/src/modules/products/templates/design-info/index.tsx` (Medusa `Heading`/`Text`/`Badge`/`IconBadge`/`Popover`, `--ui-*` tokens, `replace(/[-_]/g," ")` + title-case humanizer). NO money.
- Raw-material `media` json shapes (mirror in storefront, can't import backend): canonical `{files:string[]}`, bare `string[]`, `{files:[{url|file_path}]}` → see `normalizeMediaUrls` in `production-story/lib.ts` (copied from backend `apps/backend/src/workflows/media/lib/media-binding.ts:normalizeMediaFiles`).
- Storefront skeleton tokens: `bg-ui-bg-component-pressed` + `animate-pulse` (see `src/modules/skeletons/components/skeleton-card-details`). No "Loading…" strings.
- ⚠ Storefront test env BROKEN: vitest@4 (config `vitest.config.ts`, include `src/**/*.test.ts`, runner `npm run test`) needs vite ≥6 but repo-root hoists `vite@5.4.21` → `ERR_PACKAGE_PATH_NOT_EXPORTED ./module-runner`. Verify pure logic via `apps/storefront/node_modules/.bin/tsx` smoke script instead.
- ⚠ `npm run build` (storefront) exits BEFORE compiling unless `NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY` is set (`check-env-variables.js`). With `NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY=pk_test` it reaches `✓ Compiled successfully`, then fails "Collecting page data" (no live backend) — compile success is the signal, prerender fail is environmental.
- ⚠ Repo-wide pre-existing `TS2786 'Text'/'Suspense' cannot be used as a JSX component` (React types dup) is on origin/main behind `ignoreBuildErrors` — IGNORE; only count errors citing YOUR new file paths.

## Partner-UI media thumbnails (raw-material photos)
- Shared util: `apps/partner-ui/src/lib/first-media-url.ts` — `firstMediaUrl(media)`/`mediaUrls(media)`; unwraps `{files:[...]}`/arrays/objects. Mirror of admin `apps/backend/src/admin/lib/utils/first-media-url.ts`.
- Linking-picker thumbnail data: hook `apps/partner-ui/src/hooks/api/partner-design-inventory.tsx` → `normalizeRawMaterialRow` sets `media_url`; rendered by `routes/designs/design-add-inventory/components/add-inventory-form/add-inventory-form.tsx` `<Thumbnail src={r.media_url}>`.
- BOM list thumbnails: `routes/designs/design-detail/components/design-inventory-bom-section.tsx` `extractMedia` (uses `mediaUrls`, up to 4).
- Backend feed: `GET /partners/inventory-items/raw-materials` (`apps/backend/src/api/partners/inventory-items/raw-materials/route.ts`) → `getAllInventoryWithRawMaterial` with `["*"]` returns `raw_materials.*` incl media. To add a media-bearing field, no fetch change needed.
- To fix a "blank thumbnail" bug: check the extraction unwraps `{files}` (use the shared util), NOT the render.

## Designs module migrations (prod re-apply pattern)
- Design-brief columns migration: `apps/backend/src/modules/designs/migrations/Migration20260622120000.ts` (concept_theme/persona/competitors/price_point/design_budget/raw_design_budget).
- Routes that select ALL design cols (500 if a col missing on prod): `src/api/admin/designs/[id]/components/route.ts` + `.../used-in/route.ts` (load component_design/parent_design relations).
- To re-apply a recorded-but-not-executed prod migration: add NEW migration in same migrations/ dir, globally-unique class name (grep `20260624XXXXXX` across ALL modules — timestamps collide cross-module), re-issue original up() idempotent DDL verbatim. Ref PR #736.

## #730 media↔raw-material bind tool (PR #737)
- Endpoint: `src/api/admin/medias/file/[id]/raw-material/route.ts` (GET binding / POST bind / DELETE unbind) + `validators.ts`. Depth from that dir to `src/` = 6 `../`.
- Pure helpers (unit-tested): `src/workflows/media/lib/media-binding.ts` — `normalizeMediaFiles`/`appendMediaFile`(idempotent)/`removeMediaFile`/`stampBinding`/`clearBinding`(nulls keys)/`readBinding`. raw_materials.media canonical shape = `{ files: string[] }`.
- Admin hook: `src/admin/hooks/api/media-raw-material-binding.ts` (useMediaRawMaterialBinding/useBind…/useUnbind…). UI: `src/admin/components/media/bind-raw-material-section.tsx`, mounted in `routes/medias/[id]/@media/components/folder-media-gallery/folder-media-gallery.tsx` right panel (Tag-icon toggle, keyed by currentMedia.id/url).
- raw_material model REQUIRES `name`+`description`+`composition` (description is `model.text()` NOT nullable) — create payloads must set description or createRawMaterials throws MikroORM ValidationError → 500.
- raw_material has NO sku; SKU is on the LINKED inventory_item (link `src/links/raw-material-data-inventory.ts`). createRawMaterialWorkflow needs an existing `inventoryId`; returns `[rawMaterialResult, link, sku]` → `result[0].id`.
- Medusa module-service `update` MERGES metadata json (does NOT replace) — to clear a key set it to null, deleting the key leaves the old value.
- List/search raw materials: hook `useInventoryWithRawMaterials({q,limit})` → `GET /admin/inventory-items/raw-materials` → `{inventory_items:[{id(sku),raw_materials:{...}}]}`.
- inventory_item.thumbnail sync (#739/#730): step `src/workflows/raw-materials/steps/sync-inventory-thumbnail.ts` (wired into create-raw-material.ts + update-raw-material.ts via WorkflowResponse); resolves inv item via `inventoryId` OR query.graph entity `inventory_item_raw_materials` filter `raw_materials_id`; sets `thumbnail=firstMediaUrl(media)` idempotent. The #737 bind route writes `raw_materials.media` DIRECTLY (no workflow) so it does NOT trigger the step — PR #740 fixes by resolving the inv item in-route + setting thumbnail via pure `src/utils/inventory-thumbnail-decision.ts` `pickInventoryThumbnail(current,media,{overwrite})` (reuses firstMediaUrl, no-clobber-by-default, best-effort try/catch). To mirror a raw-material photo onto inventory thumbnail from a NON-workflow path, call that pure helper + `inventoryService.updateInventoryItems([{id,thumbnail}])`.
