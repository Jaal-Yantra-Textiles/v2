...
// Free-model delegation: the daemon may hand well-scoped, low-risk subtasks to a
// cheap opencode drafter (scripts/agent-daemon/delegate.mjs) and verify the output
...
RESUME (first, in order):
  ...
  6. Read scripts/agent-daemon/CODEBASE_MAP.md — the durable map of WHERE things live (file paths,
     route/symbol locations) + per-pattern "to do X, edit Y" recipes from past chunks. TRUST it to jump
     straight to the right files instead of re-grepping; only search/re-derive when the map lacks the
     answer or a referenced path is clearly stale. Re-discovering what's already mapped is the main cost
     sink — don't.
...
${
  DELEGATE
    ? `
DELEGATE (optional, to save tokens): you have a FREE drafting sub-agent — an opencode run on ${DELEGATE_MODEL}.
For a WELL-SCOPED, LOW-RISK subtask you can fully specify in a prompt — a throwaway Playwright test script, a
small admin UI section/component from a precise spec, a mechanical/boilerplate edit (mirror an existing route,
add a field to a schema+form), or a unit/integration test STUB for code you just wrote — you MAY delegate the
first draft instead of writing it yourself:
  node scripts/agent-daemon/delegate.mjs --files "<comma-separated paths it may touch>" "<precise, self-contained task>"
It AUTHORS FILES ONLY — git, installs, and network are hard-blocked; it never commits/pushes/merges. YOU are the
VERIFIER and own correctness: after it returns, read scripts/agent-daemon/delegate-logs/last-result.json (changed[]
+ out_of_scope[]), review \`git diff -- <files>\`, typecheck the changed files (zero NEW errors), run the relevant
PER-FILE spec or drive Playwright, and FIX it or \`git checkout -- <file>\` to DISCARD if it's wrong or out of scope.
A clean exit is NOT a correct result — never let an unverified draft ship. Delegate only mechanical work; do the
judgement-heavy parts (architecture, money/cost logic, migrations, security, anything in scripts/agent-daemon/)
YOURSELF. If the drafter returns DRAFT_BLOCKED or strays out of scope, just do it yourself.

ANALYSIS CHUNKS can ALSO delegate the first draft of a per-module BEHAVIOUR doc (the drafter reads the module and
writes a grounded doc that cites \`path:symbol\` for every claim):
  node scripts/agent-daemon/delegate.mjs --mode analysis --files "apps/docs/notes/<area>-behaviour.md" "<what module to document>"
Then VERIFY it mechanically — \`node scripts/agent-daemon/verify-analysis.mjs <doc>\` checks every cited file/symbol exists
(a missing file = hallucination, exit 1) — then read the prose + routes yourself and correct or DISCARD. The free model
is hallucination-prone on analysis, so this verify step is mandatory; you own the truth of the doc.
`
    : ""
}
SHIP (build chunks): \`git fetch origin\`. Branch off \`origin/main\` by DEFAULT (single workspace — no
worktrees). STACK INSTEAD when this slice is the next in a SERIES of similar slices that all edit the SAME
central registration file (e.g. the \`src/api/middlewares.ts\` matcher list, a \`MAINTENANCE_JOBS\`-style
registry array, a shared barrel/index) AND the prior slice's PR is still OPEN/unmerged: branch off that
prior slice's branch (not origin/main) and set the PR base to it, so the shared-file edits append in
sequence and never textually conflict. Title/handoff must note "stacked on #<prior> (merge parent-first)".
Once the series' base PR merges, later independent slices go back to branching off origin/main. ${
  MERGE
...
Also update scripts/agent-daemon/CODEBASE_MAP.md with any code-search result you had to DERIVE this chunk
that a future fresh-context chunk would otherwise re-discover: file paths, route/symbol locations, and a
one-line "to add X, edit Y" recipe. Keep it a CONCISE index (paths + one-liners, NEVER code dumps); correct
or prune stale entries. This is the anti-redundancy ledger — cheaper next-chunk discovery is the whole point.
...
handoff and END your final message with the exact token ${DONE} ALONE on the final line (nothing after it).
Never write the literal token in prose/negation ("not done" etc.) — it is a sentinel; only emit it to stop. Otherwise end normally.
...
async function main() {
  console.log(
    `[daemon] repo=${REPO} mode=${MODE} maxChunks=${MAX_CHUNKS} budget=$${BUDGET_USD} ` +
      ...
      `delegate=${DELEGATE ? DELEGATE_MODEL : "off"}`
  )
  let total = 0
  let consecErrors = 0
  let ranOk = 0
  ...
  for (let n = 1; n <= MAX_CHUNKS; n++) {
    ...
    // Match the token only as its own line (the agent is told to END with it
    // alone). A substring match falsely fired on prose like "not emitting
    // DAEMON_DONE", quitting the wave after one chunk with budget to spare.
    if (new RegExp(`^\\s*${DONE}\\s*$`, "m").test(r.finalText)) {
